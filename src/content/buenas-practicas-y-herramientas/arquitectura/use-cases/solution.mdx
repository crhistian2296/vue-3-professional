...
A continuación verás cómo mover la lógica de `fetch` fuera del componente y encapsularla en un **Use Case**. Lo ejecutaremos a través de un **UseCaseService**. Para el ejercicio, usaremos una infraestructura mínima ya copiada en esta carpeta (`./core/use-cases`), y puedes tomar como referencia adicional `arquitectura/core` si quieres profundizar.
...
Puntos clave:
- La clase `GetTodoQry` implementa `Query<TodoDto, { id: number }>` y expone el método `handle`.
...
```ts
// ./core/use-cases/* (infra mínima incluida en el ejercicio)
...
Si lo prefieres, puedes empezar solo con `EmptyMiddleware` y añadir el logger más adelante. También puedes consultar la versión completa en `arquitectura/core` para profundizar.
...
import { EmptyMiddleware } from './use-cases/middlewares/empty.middleware'
import { LoggerMiddleware } from './use-cases/middlewares/logger.middleware'
import { GetTodoQry } from '../application/get-todo.qry'

const middlewares = [new LoggerMiddleware(), new EmptyMiddleware()]
...
### 6) Variantes y siguientes pasos

- Sustituir el `fetch` interno por un `HttpClient` inyectado (ver `core/core/http-client/http-client.ts`).
- Añadir middlewares reales como en `core/core/use-cases/middlewares` y componerlos en el `UseCaseService`.
- Manejar ids dinámicos (por ejemplo, mediante inputs de usuario o parámetros de ruta).
